...
class HashTable:
    ...
    def __getitem__(self, key):
        """
        Gets an item from the hashtable with the specified key (QUADRATIC PROBING)
        ...
        offset = 0
        while self.table[(location+offset**2) % self.length] is not None:
            if self.table[(location+offset**2) % self.length][0] == key:
                return self.table[(location+offset**2) % self.length][1]
            offset += 1
        raise KeyError

    def __setitem__(self, key, item):
        """
        Sets an item from item into the hashtable with the specified key (QUADRATIC PROBING)
        ...
        if self.__contains__(key):
            while self.table[(index + offset**2) % self.length][0] != key:
                print("?")
                offset += 1

        else:
            while self.table[(index + offset**2) % self.length] is not None:
                offset += 1

        if offset > 0:
            if offset > self.probe_max:
                self.probe_max = offset

            self.probe_total += offset
            self.collisions += 1

        self.count += 1
        self.table[(index + offset**2) % self.length] = [key, item]

    def __contains__(self, key):
        """
        Checks to see if a key exists in the hashtable (QUADRATIC PROBING)
        ...
        offset= 0
        while self.table[(location + offset**2) % self.length] is not None:
            #print((location + offset**2) % self.length)
            if self.table[(location + offset**2) % self.length][0] == key:
                return True
            offset += 1
        return False
...
def table_load_dictionary_statistics(max_time):
    ...
    the output_task4.csv file

    :pre condition: max_time must be an integer, and there must be files available to read from
    :post condition: output_task4.csv will contain rows for each combination of data and the results produced
    from them.
    ...
    with open('output_task4.csv', 'w', newline='') as f:
